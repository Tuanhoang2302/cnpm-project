...
export var isWannaReset8 = [false];
...
export default class Scene2v2 extends Phaser.Scene
{
    ...
    create(){
        ...
        for(var i = 0; i < 6; i++){
            console.log(CountAppleEachBox2[i]);
            
        }
    }
   
    update(){
        isWannaReset8[0] = this.isWannaReset[0];
        this.DragAndDrop();
    ...
    DisplayAppleInBox(){
        ...
        if(this.isDisplayBox){
            ...
            CountAppleEachBox2[this.boxIndex] = 11;
    ...
    DisplayResult(){
        if(this.isDisplayResult){
            this.checkInput.check(this.msg.graphicArr[0], this.msg.ContentArr[0],this.inputText[0], this.isRightResult, this.isWannaReset, 40);
            if(boxDisappear[0]){
                for(var k = 0; k < 6; k++){
                    if(CountAppleEachBox2[k] != 11){              
                        var x = k % 3;
                        var y = k / 3;
            ...
            if(this.isRightResult[0]){
                ...
                var text = this.add.text(RESULTTXT.X , RESULTTXT.Y, '40', { fontFamily: 'Arial', fontSize: 60, color: '#000000', align: 'center', wordWrap: { width: 30 }});
    ...
    CreateGameFunction(){
        ...
        this.boxDisplay.displayMessageBox(MESSAGE.X, MESSAGE.Y, MESSAGE.WIDTH, MESSAGE.HEIGHT,
            'There are 40 blocks', MESSAGE.FONTTEXT, this.msg.graphicArr, this.msg.ContentArr);
        for(var i = 0; i < this.appleArr.length; i++){
            this.initApplePosX.push(this.appleArr[i].x);
            this.initApplePosY.push(this.appleArr[i].y);
            this.distance.push(0);
        }
        var dragManager = new DragManager(this, this.appleArr, this.Holder, this.initApplePosX, this.initApplePosY, 2);
    ...
    ReCreate(){
        ...
        this.fade = null;
        CountAppleEachBox2 = [0, 0, 0, 0 , 0 ,0]
        isWannaReset8 = [false]
        spaceValid2 = [];
    }
}